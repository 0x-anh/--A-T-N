...
import { ChevronDown, Check, Plus, UserPlus, Trash2, Languages } from 'lucide-react';
      ...
      <div className="flex items-center gap-6">
        {(activeTab === 'dashboard' || activeTab === 'metrics' || activeTab === 'logs') ? (
          <div className="flex items-center gap-3 px-4 py-1.5 bg-slate-900 text-white rounded-lg shadow-lg shadow-slate-900/10 border border-slate-800">
             ...
             <span className="text-[10px] font-black uppercase tracking-[0.2em] font-mono">{t('topbar.general_system')}</span>
          </div>
        ) : null}
        ...
        {!['dashboard', 'metrics', 'logs'].includes(activeTab) && (
          <div className="relative">
            ...
            >
               ...
               <span className="text-[11px] font-bold text-slate-900 uppercase tracking-tight truncate max-w-[150px]">{selectedProject?.name || t('topbar.loading')}</span>
            ...
            <AnimatePresence>
               {showProjectDropdown && (
                 ...
                 >
                   <div className="px-3 py-1.5 text-[9px] font-bold text-slate-400 uppercase tracking-widest">{t('topbar.projects')}</div>
                   <div className="space-y-0.5 max-h-[300px] overflow-y-auto custom-scrollbar">
                     ...
                     >
                       <Plus size={14} /> {t('topbar.create_project')}
                     </button>
                   </div>
                 </motion.div>
               )}
        ...
        <div className="flex items-center gap-2">
           <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
             {activeTab === 'dashboard' ? t('sidebar.dashboard') : activeTab === 'metrics' ? t('sidebar.metrics') : activeTab === 'logs' ? t('sidebar.logs') : activeTab}
           </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2 mr-2">
           <button 
             ...
             className="h-9 px-3 bg-white border border-slate-200 rounded-lg text-[10px] font-black text-slate-900 hover:bg-slate-50 transition-all flex items-center gap-2 shadow-sm uppercase tracking-tighter"
             title={t('topbar.language')}
           >
              <Languages size={14} className="text-brand-500" />
              <span className="w-5 text-center">{i18n.language === 'vi' ? 'EN' : 'VI'}</span>
           </button>
        </div>
        ...
        {(isAdmin || selectedProject?.ownerId === userId) && (
          <button onClick={() => setShowInviteModal(true)} className="h-9 px-4 bg-slate-950 text-white rounded-lg text-xs font-bold hover:bg-brand-600 transition-all flex items-center gap-2">
             <UserPlus size={14} /> 
             <span className="hidden sm:inline">{t('kanban.team')}</span>
          </button>
        )}
      </div>
    </header>
